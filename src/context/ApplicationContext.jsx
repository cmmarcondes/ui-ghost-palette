...
const ApplicationContextProvider = ({ children, ...props }) => {
  ...
  function handleChangeSong(event, loadSound) {
    const { files } = event.target;
    const uploadedSong = new loadSound.loadSound(files[0]);
  ...
  function preparePalette(arrayDivider, frequencyArray) {
    ...
    for (let i = 0; i < NUMBER_OF_COLORS_IN_PALETTE; i++) {
      const palette = frequencyArray.slice(
        arrayDivider * i,
        arrayDivider * (i + 1)
      );
      updatedPaletteArray.push(
        palette.reduce(
    ...
    const stringFormatedPalette = uploadedHexStringArray.map((string) =>
      string.slice(0, 6)
    );
